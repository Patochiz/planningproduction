...
<body>
    ...
    function openCardModal(btn) {
        ...
        if (fpCheckbox) fpCheckbox.checked = (btn.dataset.fpTransmise === 'oui' || btn.dataset.fpTransmise === '1');
...
function renderCardsTable($cards, $langs)
{
    ...
    foreach ($cards as $card) {
        ...
        $fp_val_tab = $card['fp_transmise'] ?? '';
        $fp_hidden_tab = ($fp_val_tab === 'oui' || $fp_val_tab === '1') ? '' : ' badge-fp-hidden';
    ...
    foreach ($planned_cards as $week => $week_data) {
        ...
        foreach ($groups as $group_name => $cards) {
            ...
            foreach ($cards as $card) {
                ...
                $btn_data .= ' data-postlaquage="' . htmlspecialchars($card['postlaquage'] ?? 'non', ENT_QUOTES) . '"';
                $btn_data .= ' data-fp-transmise="' . htmlspecialchars($card['fp_transmise'] ?? 'non', ENT_QUOTES) . '"';
                $fp_val_plan = $card['fp_transmise'] ?? '';
                $fp_hidden_plan = ($fp_val_plan === 'oui' || $fp_val_plan === '1') ? '' : ' badge-fp-hidden';
